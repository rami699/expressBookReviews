...
public_users.get('/isbn/:isbn', async function (req, res) {
    try {
        ...
        const book = await getBookWithISBN();
...
public_users.get('/author/:author', async function (req, res) {
    try {
        ...
        res.status(200).json({ message: authorBooks });
    } catch (err) {
...
public_users.get('/title/:title', async function (req, res) {
    try {
        ...
        const book = await getBooksByTitle();
        res.status(200).json({ message: book });
    } catch (err) {
        res.status(404).json({ message: err });
    }
...
public_users.get('/isbn/:isbn',function (req, res) {
  const isbn = req.params.isbn
  const book = books[isbn]
  const booksString = JSON.stringify(book, null, 4);
    return res.status(200).json({message: booksString});
 });
